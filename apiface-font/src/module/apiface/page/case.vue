 ...
 */
<template>
  <self-section class="blue" :label="$self('label')" v-show="data && data.show">
    ...
    <div class="time-info">
      ...
      <json-viewer :value="data.requestHeader" :expand-depth="5" copyable boxed sort></json-viewer>
    </div>
    <div class="time-info">
      <div class="time-info-title">{{ $self("respHead") }}</div>
      <json-viewer :value="data.responseHeader" :expand-depth="5" copyable boxed sort></json-viewer>
    </div>
    <div class="time-info">
      <div class="time-info-title">{{ $self("body") }}</div>
      <json-viewer :value="data.result" :expand-depth="5" copyable boxed sort></json-viewer>
    </div>
  </self-section>
</template>
