...
<template>
  ...
  >
    <!--  @expandRow="expandRow" -->
    <datagrid :data="data" rowkey="key" :expand="false">
      <el-table-column
        prop="enable"
        :label="$t('enable')"
        width="60"
      ...
      >
        <template slot-scope="scope">
          <div class="el-table__inner-self" v-if="scope.row.type == 'json'">
            <datagrid :data="scope.row.schema.data.child" rowkey="key">
              <el-table-column
                prop="name"
                label="#"
                width="160"
...
export default {
  ...
  methods: {
    ...
    feel() {
      this.$emit("feelback", this.data, "request");
    },
    checkChange(flag) {
      if (flag) {
        //改变状态并计算值
