...
### 演示地址
* 演示地址: <br/>
<a href="https://dayeshing.github.io/apiface/apiface.html" target="_blank">apiface 静态（纯前端）</a><br/>
<a href="http://apiface.daysh.top/index" target="_blank">apiface动态（包含后端</a>
* 角色用户(登录账号@密码): <br/>
<span>文档管理者</span>:<span>manager@123456</span> <br/>
<span>文档使用者</span>:<span>document@123456</span> <br/>
...
```
##### <a id="javadocs">apiface的javadoc标记规范</a>  (非常重要)
...
|@group|分组标记|@group groupName,groupName2|method|含@action的文档注释|对应model @required 分组 标记属性字段是否必须|
...
|@required|是否必须|@required groupName,groupName2|(get)method,field|含@model的文档注释|标记引用该model的接口类的@group groupName,groupName2中属性字段是否必须|
...
##### <a id="apifaceMavenPluginUsing">apiface-maven-plugin使用方式</a>（代码编译扫描，非常重要）
```xml
<!--
...
##### <a id="apifaceGradlePluginUsing">apiface-gradle-plugin使用方式</a>（代码编译扫描，非常重要）
```groovy
/**
 * 插件已发布至中央仓库，可以直接从中央仓库获取 插件以及插件依赖，  认准 1.4.5-RC 版本及其以上版本
 */
